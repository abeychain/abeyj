...
import org.abeyj.abi.datatypes.primitive.Byte;
import org.abeyj.abi.datatypes.primitive.Double;
import org.abeyj.abi.datatypes.primitive.Float;
import org.abeyj.abi.datatypes.primitive.Int;
import org.abeyj.abi.datatypes.primitive.Long;
import org.abeyj.abi.datatypes.primitive.Short;
import org.abeyj.abi.datatypes.primitive.*;
...
import org.abeyj.protocol.core.RemoteFunctionCall;
import org.abeyj.protocol.core.methods.request.AbeyFilter;
import org.abeyj.protocol.core.methods.response.AbiDefinition;
import org.abeyj.protocol.core.methods.response.AbiDefinition.NamedType;
import org.abeyj.protocol.core.methods.response.BaseEventResponse;
...
import org.abeyj.utils.Collection;
import org.abeyj.utils.Strings;
import org.abeyj.utils.Version;
...
public class SolidityFunctionWrapper extends Generator {
    ...
    private static final String ABEYJ = "abeyj";
    ...
    private void addAddressesSupport(TypeSpec.Builder classBuilder, Map<String, String> addresses) {
        if (addresses != null) {
            ...
            // See org.abeyj.tx.Contract#getStaticDeployedAddress(String)
    ...
    private TypeSpec.Builder createClassBuilder(
        ...
        String javadoc = CODEGEN_WARNING + getAbeyjVersion();
    ...
    private String getAbeyjVersion() {
        String version;

        try {
            // This only works if run as part of the abeyj command line tools which contains
            // a version.properties file
            version = Version.getVersion();
        } catch (IOException | NullPointerException e) {
            version = Version.DEFAULT;
        }
        return "\n<p>Generated with abeyj version " + version + ".\n";
    }
    ...
    private static MethodSpec buildConstructor(
        ...
        MethodSpec.Builder toReturn =
                MethodSpec.constructorBuilder()
                        ...
                        .addParameter(Abeyj.class, ABEYJ)
     ...
     * <code>List<struct></code> class Else, we return the Abeyj generated types.
     *
    ...
    private TypeName buildStructArrayTypeName(NamedType namedType, Boolean useNativeJavaTypes) {
        ...
        if (!arrayLength.isEmpty() && Integer.parseInt(arrayLength) > 0) {
            return ParameterizedTypeName.get(
                    ClassName.get("org.abeyj.abi.datatypes.generated", "StaticArray" + arrayLength),
    ...
    private void buildConstantFunction(
        ...
        if (outputParameterTypes.isEmpty()) {
            ...
            ParameterizedTypeName parameterizedTupleType =
                    ParameterizedTypeName.get(
                            ClassName.get(
                                    "org.abeyj.tuples.generated", "Tuple" + returnTypes.size()),
    ...
    TypeSpec buildEventResponseObject(
        ...
        for (org.abeyj.codegen.SolidityFunctionWrapper.NamedTypeName namedType :
                indexedParameters) {
            final TypeName typeName;
            if (namedType.getType().equals("tuple")) {
    ...
    MethodSpec buildEventFlowableFunction(
        ...
        MethodSpec.Builder flowableMethodBuilder =
                MethodSpec.methodBuilder(generatedFunctionName)
                        .addModifiers(Modifier.PUBLIC)
                        .addParameter(AbeyFilter.class, FILTER)
        ...
        flowableMethodBuilder.addStatement(
                "return abeyj.ethLogFlowable(filter).map($L)", converter);
    ...
    private static Class<?> getStaticArrayTypeReferenceClass(String type) {
        try {
            return Class.forName("org.abeyj.abi.datatypes.generated.StaticArray" + type);
