...
public class AbeyjDelegate {
    ...
    public static MyDelegateResult getDelegate(String address, String holderAddress) {
        ...
        org.abeyj.protocol.core.methods.request.Transaction transaction =
                org.abeyj.protocol.core.methods.request.Transaction.createEthCallTransaction(address, stakingContentAddress, data);
        ...
        try {
            AbeyCall abeyCall = stakingAbeyj.abeyCall(transaction, DefaultBlockParameterName.LATEST).sendAsync().get();
    ...
    public static String sendFunctionTransfer(String hexValue) {
        String txHash = null;
        try {
            AbeySendTransaction abeySendTransaction = stakingAbeyj.abeySendRawTransaction(hexValue).sendAsync().get();
            ...
            txHash = abeySendTransaction.getTransactionHash();
    ...
    public static String sendFunctionTransferAndCHeckValue(String hexValue) {
        ...
        try {
            ...
            if (txHash != null) {
                Transaction tr = stakingAbeyj.abeyGetTransactionByHash(txHash).sendAsync().get().getResult();
                if (null != tr) {
    ...
    public static BigDecimal getAbeyBalance(String address) {
        try {
            AbeyGetBalance abeyGetBalance1 = stakingAbeyj.abeyGetBalance(address, DefaultBlockParameter.valueOf("latest")).send();
            return new BigDecimal(abeyGetBalance1.getBalance()).divide(new BigDecimal("1000000000000000000"));
        } catch (Exception e) {
    ...
    public static BigInteger queryTxExecutionReceipt(String txHash) {
        //ȷ�Ͻ�����ִ��
        AbeyGetTransactionReceipt receipt = null;
        try {
            receipt = stakingAbeyj.abeyGetTransactionReceipt(txHash).send();
    ...
    public static String staking_delegate(BigInteger value) {
        ...
        BigInteger nonce = NonceManager.getInstance().getNonce(stakingAbeyj, credentials.getAddress());
        ...
        try {
            abeySendTransaction = stakingAbeyj.abeySendRawTransaction(hexValue).sendAsync().get();
            System.out.println("createOrder: tx " + abeySendTransaction.getTransactionHash());
            NonceManager.getInstance().exhaustNonce(credentials.getAddress(), abeySendTransaction.getTransactionHash());
...
//        String b = "";
//        try {
//            b = getAbeyTestBalance(credentials.getAddress()).toPlainString();
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
...
//        System.out.println(b);
    ...
    public static BigInteger staking_getDelegate() {
        ...
        String data = FunctionEncoder.encode(function);
        org.abeyj.protocol.core.methods.request.Transaction transaction =
                org.abeyj.protocol.core.methods.request.Transaction.createEthCallTransaction(credentials.getAddress(), stakingContentAddress, data);
